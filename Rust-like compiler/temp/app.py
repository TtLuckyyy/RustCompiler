...
from temp.lexer import Tokenize
...
class GrammarVisualizerApp:
    ...
    def create_widgets(self):
        ...
        # 插入语法产生式
        idx = 0
        for lhs in self.parser.prod_map.keys():
            for prod in self.parser.prod_map[lhs]:  
                rhs = prod['rhs']
    ...
    def analyze_code(self):
        ...
        try:
            ...
            ast_root, trace = self.parser.parse(tokens=tokens, checker=self.checker)
            # 统一 key
            for step in trace:
                if "prod" in step:
                    step["production"] = step.pop("prod")
            self.analysis_details = trace
    ...
    def record_parsing_process(self, tokens):
        ...
        while True:
            ...
            action = self.parser.action_tbl[state].get(current_token.type.value)
            if not action:
                expected = sorted(self.parser.action_tbl[state].keys())
            ...
            if action[0] == 'shift':
                ...
                step_info["production"] = ""
            elif action[0] == 'reduce':
                prod_index = action[1]
                prod = self.parser.prod_idx_map[prod_index]
                lhs = prod['lhs']
    ...
    def show_step(self, step_index):
        ...
        stack_str = " ".join(f"[{state}]" for state in step["state_stack"])  # 这里改为 state_stack
    ...
    def show_tables(self):
        ...
        try:           
            ...
            # 设置列
            terminals = sorted({k for state in self.parser.action_tbl.values() for k in state.keys()})
            ...
            # 填充数据
            for state in sorted(self.parser.action_tbl.keys()):
                values = [self.parser.action_tbl[state].get(term, "") for term in terminals]
